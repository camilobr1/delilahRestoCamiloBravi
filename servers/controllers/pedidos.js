...
function newOrder(req, res) {
  ...
  let sql = `INSERT INTO delilahrestocamilo.pedidos(user_id, status, payment_method, update_time)
  ...
  connection.query(sql, (err, orders) => {
    if (err) {
      ...
      order.products.forEach((element) => {
        let orderUnity = `INSERT INTO delilahrestocamilo.info_pedidos(order_id, product_id, quantity)
...
function updateOrder(req, res) {
  ...
  if (userRol == 1) {
    sql = `UPDATE delilahrestocamilo.pedidos
