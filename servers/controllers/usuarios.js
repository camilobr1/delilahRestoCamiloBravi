...
function creaUsuario(req, res) {
  ...
  let validator = "SELECT email, username FROM delilahrestocamilo.usuarios";
  ...
  connection.query(validator, function (err, info) {
    ...
    let sql = `INSERT INTO delilahrestocamilo.usuarios(username, fullname, email, phone, address, password, is_admin)
...
function logIn(req, res) {
  let user = req.body;
  let token = jwt.sign(user.password, firma);
  let sql = `SELECT password FROM delilahrestocamilo.usuarios WHERE usuarios.password = '${token}' AND usuarios.username = '${user.username}'`;
...
function retornaUsuario(req, res) {
  ...
  if (userRol == 1) {
    ...
    sql = "SELECT * FROM delilahrestocamilo.usuarios";
  } else {
    console.log(`Cuenta de Usuario`);
    sql = `SELECT * FROM delilahrestocamilo.usuarios WHERE id = ${userId}`;
  }
